...
from UE_Sequence_Assemble import path       #资产路径处理模块 包括UE内部资产和服务器镜头资产
from UE_Sequence_Assemble import asset         #UE资产读取模块
from UE_Sequence_Assemble import sequence    #UE组装功能模块
        ...
        for i in file_lists_ins:
            ...
            if cam_path_dict !={}: #判定相机是否存在 
                ...
                if file_name.split('_')[4] == 'Camera':
                    ...
                    ch_pr = file_path
                    camera = None
                self.sequence = sequence.Sequence(cam_file_lists[0],seq_clear,camera,ch_pr)  #实例化sequence 执行镜头组装
