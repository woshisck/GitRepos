...
# 导入包里的四个功能模块
from ANI_Cha_Assemble.add_UI import Ui_Seq_Assemble
from ANI_Cha_Assemble import Path_Mod       #资产路径处理模块 包括UE内部资产和服务器镜头资产
from ANI_Cha_Assemble import Get_Asset         #UE资产读取模块
from ANI_Cha_Assemble import Seq_Mod    #UE组装功能模块
...
class main_window(QWidget,Ui_Seq_Assemble):
    def __init__(self,parent=None):
        ...
        # 实例化两个功能类  
        self.path = Path_Mod.Folder_path()
        ...
        for i in file_lists_ins:
            ...
            if cam_path_dict !={}: #判定相机是否存在 
                ...
                if file_name.split('_')[4] == 'Camera':
                    ...
                    ch_pr = file_path
                    camera = None
                self.sequence = Seq_Mod.Sequence(cam_file_lists[0],seq_clear,camera,ch_pr)  #实例化sequence 执行镜头组装
                self.sequence.Seq_merge()#执行组装动作。
            else:
