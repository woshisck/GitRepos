...
def get_BS_mesh(check_bs_mode_grp):
    """
    get blendshape mesh list and bake blendshape node
    check_bs_mode_grp = mode_HightMesh_grp_name
    """
    BS_mesh_list = []
...
def setExportFlags():
    ...
    mm.eval("FBXExportHardEdges -v true")
    ...
    mm.eval("FBXExportBakeResampleAnimation -v true")
...
def get_out_cha_name(cha_name):
    ...
    if len(asset_name)==5:
        ...
        if "HIK" in asset_name[-1] or "Mocap" in asset_name[-1]:
